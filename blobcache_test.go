...
func TestCoalesceVictims(t *testing.T) {
	tests := []struct {
		name     string
		victims  []index.Item
		expected []HoleRange
		...
		},
		{
			name: "single victim",
			victims: []index.Item{
				{SegmentID: 1, Offset: 100, PhysicalLen: 50},
			},
			expected: []HoleRange{
				{SegmentID: 1, Offset: 100, Length: 50},
			},
		},
		{
			name: "two contiguous same segment",
			victims: []index.Item{
				{SegmentID: 1, Offset: 100, PhysicalLen: 50},
				{SegmentID: 1, Offset: 150, PhysicalLen: 30}, // Contiguous: 100+50=150
			},
			expected: []HoleRange{
				{SegmentID: 1, Offset: 100, Length: 80}, // Merged: 50+30
			},
		},
		{
			name: "two non-contiguous same segment",
			victims: []index.Item{
				{SegmentID: 1, Offset: 100, PhysicalLen: 50},
				{SegmentID: 1, Offset: 200, PhysicalLen: 30}, // Gap at 150-200
			},
			expected: []HoleRange{
				{SegmentID: 1, Offset: 100, Length: 50},
				{SegmentID: 1, Offset: 200, Length: 30},
			},
		},
		{
			...
			victims: []index.Item{
				{SegmentID: 1, Offset: 100, PhysicalLen: 50},
				{SegmentID: 2, Offset: 100, PhysicalLen: 30},
			},
			expected: []HoleRange{
				{SegmentID: 1, Offset: 100, Length: 50},
				{SegmentID: 2, Offset: 100, Length: 30},
			},
		},
		{
			name: "three contiguous merges into one",
			victims: []index.Item{
				{SegmentID: 1, Offset: 1000, PhysicalLen: 100},
				{SegmentID: 1, Offset: 1100, PhysicalLen: 100},
				{SegmentID: 1, Offset: 1200, PhysicalLen: 100},
			},
			expected: []HoleRange{
				{SegmentID: 1, Offset: 1000, Length: 300},
			},
		},
		{
			name: "unsorted input gets sorted",
			victims: []index.Item{
				{SegmentID: 2, Offset: 200, PhysicalLen: 30},
				{SegmentID: 1, Offset: 150, PhysicalLen: 50}, // Should sort before seg 2
				{SegmentID: 1, Offset: 100, PhysicalLen: 50}, // Contiguous with 150
			},
			expected: []HoleRange{
				{SegmentID: 1, Offset: 100, Length: 100}, // 100-200 merged
				{SegmentID: 2, Offset: 200, Length: 30},
			},
		},
		{
			name: "mixed: some contiguous, some not",
			victims: []index.Item{
				{SegmentID: 1, Offset: 100, PhysicalLen: 50},
				{SegmentID: 1, Offset: 150, PhysicalLen: 50}, // Contiguous with 100
				{SegmentID: 1, Offset: 300, PhysicalLen: 50}, // Gap
				{SegmentID: 2, Offset: 0, PhysicalLen: 100},
				{SegmentID: 2, Offset: 100, PhysicalLen: 100}, // Contiguous in seg 2
			},
			expected: []HoleRange{
				{SegmentID: 1, Offset: 100, Length: 100},
				{SegmentID: 1, Offset: 300, Length: 50},
				{SegmentID: 2, Offset: 0, Length: 200},
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Make a copy since CoalesceVictims sorts in place
			victims := make([]index.Item, len(tt.victims))
			copy(victims, tt.victims)

			// Test with nil buffer (should allocate)
			result := CoalesceVictims(victims, nil)
			...
			require.Equal(t, len(tt.expected), len(result), "wrong number of ranges")
			for i, exp := range tt.expected {
				require.Equal(t, exp.SegmentID, result[i].SegmentID, "SegmentID mismatch at %d", i)
				require.Equal(t, exp.Offset, result[i].Offset, "Offset mismatch at %d", i)
				require.Equal(t, exp.Length, result[i].Length, "Length mismatch at %d", i)
			}
		})
	}
}

func TestCoalesceVictims_BufferReuse(t *testing.T) {
	// Verify buffer reuse to avoid allocations
	victims := []index.Item{
		{SegmentID: 1, Offset: 100, PhysicalLen: 50},
		{SegmentID: 1, Offset: 150, PhysicalLen: 50},
	}

	// Pre-allocate buffer with capacity
	buf := make([]HoleRange, 0, 100)

	result := CoalesceVictims(victims, buf)

	// Should reuse underlying array
	require.Equal(t, 1, len(result))
	require.Equal(t, 100, cap(result), "should preserve original capacity")
}
