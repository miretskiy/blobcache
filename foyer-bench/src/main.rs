...
//!
//! Uses same workload distribution as BlobCache for fair comparison:
//!
//! # Workload Distribution (30/30/30/10)
//! - 30% Write (100KB - 2MB per blob)
//! - 30% Hot Read (Zipfian: top 10-15% of keys = 60-70% of accesses)
//! - 30% Cold Read (sequential scan pattern)
//! - 10% Miss (negative lookups)
//!
...
//! # Usage
//! ```bash
//! cargo run --release -- -n 1000000 --path /instance_storage/foyer-bench
//! ```
...
// Token bucket: 2GB burst capacity (matches Go MaxInflightSlabs(32) * WriteBufferSize(64MB))
const INFLIGHT_LIMIT_BYTES: usize = 2 * 1024 * 1024 * 1024;
...
async fn run_worker(
    worker_id: usize,
    cache: Arc<HybridCache<Bytes, Bytes>>,
    ...
    // Local histograms for this worker (merged into global at end)
    ...
    for _ in 0..iterations_per_worker {
        ...
        while !data_written {
            ...
            if adjusted_op < WRITE_BOUND {
                ...
                let key = Bytes::from(format!("key-{}", id).into_bytes());

                // Zero-copy: Bytes uses refcounting, so foyer's internal clones
                // (memory cache + storage queue) are O(1) instead of O(n) memcpy.
                let offset: usize = rng.gen_range(0..entropy.len().saturating_sub(blob_size));
                let value = Bytes::copy_from_slice(&entropy[offset..offset + blob_size]);
                ...
                if max_id > 4 {
                    ...
                    for i in 0..4u64 {
                        let key = Bytes::from(format!("key-{}", base_id + i).into_bytes());
                ...
                let key = Bytes::from(format!("miss-{}", miss_id).into_bytes());
...
async fn main() -> anyhow::Result<()> {
    ...
    println!("=== Foyer Benchmark (BlobCache-aligned config) ===");
    ...
    println!();
    println!("Configuration (matching BlobCache/Go):");
    println!("  Memory tier: 6GB (matches slab pool: 96 slabs x 64MB)");
    ...
    // Match BlobCache configuration:
    // - flush_concurrency: 2
    // - write_buffer_size: 64MB
    // - max_inflight_slabs: 32 (32 x 64MB = 2GB inflight)
    // - max_cached_slabs: 64 (64 x 64MB = 4GB cached)
    // Total slab pool: ~6GB
    ...
    let cache: Arc<HybridCache<Bytes, Bytes>> = Arc::new(
        HybridCacheBuilder::new()
            ...
            .memory(6 * 1024 * 1024 * 1024)                   // 6GB memory (match slab pool)
            .with_weighter(|_k: &Bytes, v: &Bytes| v.len())
            .storage()
    ...
    for i in 0..WARMUP_KEYS {
        ...
        let value = Bytes::copy_from_slice(&entropy[..blob_size]);
        cache.insert(key, value);
    ...
    for worker_id in 0..num_workers {
        ...
        let limiter_clone = Arc::clone(&inflight_limiter);
