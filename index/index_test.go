...
func TestIndex_PersistenceRecovery(t *testing.T) {
	...
	hash := uint64(888)
	err = idx.IngestBatch(42, []metadata.BlobRecord{{Hash: hash, Size: 99}})
	require.NoError(t, err)
	idx.Close()
...
func TestIndex_SegmentDeletion(t *testing.T) {
	...
	// Fill two segments
	require.NoError(t, idx.IngestBatch(1, []metadata.BlobRecord{{Hash: 10, Size: 100}}))
	require.NoError(t, idx.IngestBatch(2, []metadata.BlobRecord{{Hash: 20, Size: 100}}))
